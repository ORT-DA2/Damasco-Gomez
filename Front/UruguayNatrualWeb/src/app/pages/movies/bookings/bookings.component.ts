...
export class BookingsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.bookingService.getAll().subscribe(
      bookingResponse =>
        this.finishGetAll(bookingResponse), (error: string) => this.showError(error));
    this.bookingService.getBy(this.id).subscribe(
      bookingResponse =>
        this.finishGetBy(bookingResponse), (error: string) => this.showError(error));
  }

  private finishGetAll(bookingResponse: BookingBasicInfo[]){
    this.bookings = bookingResponse;
    console.log(JSON.stringify(this.bookings));
  }
  private finishGetBy(bookingResponse: BookingDetailInfo){
